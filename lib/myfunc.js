...
exports.GIFBufferToVideoBuffer = async (image) => {
    try {
        ...
        const gifPath = `./system/${filename}.gif`;
        const mp4Path = `./system/${filename}.mp4`;

        // Write GIF buffer to file
        await fs.promises.writeFile(gifPath, image);

        // Convert GIF to MP4 using FFmpeg
        await exec(`ffmpeg -i ${gifPath} -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ${mp4Path}`);

        // Wait for conversion to finish
        await new Promise(resolve => setTimeout(resolve, 4000));

        // Read MP4 file back into buffer
